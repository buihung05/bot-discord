...
function getGuildEmoji(guild, name) {
...
const commands = [
    ...
    new SlashCommandBuilder().setName('diem-danh').setDescription('Điểm danh nhận 10.000 cá'),
...
client.on('interactionCreate', async interaction => {
    ...
    const vetify = getGuildEmoji(interaction.guild, 'Vetify');
    const ewno = getGuildEmoji(interaction.guild, 'ewno');
    const coin = getGuildEmoji(interaction.guild, 'coin');
    ...
    if (interaction.commandName === 'diem-danh') {
        ...
        if (lastCheck === today) return interaction.reply(makeEmbed(`Bạn đã điểm danh hôm nay rồi! ${ewno}`));

        await db.add(`balance_${userId}`, 1000000);
        await db.set(`lastCheck_${userId}`, today);
        return interaction.reply(makeEmbed(`Điểm danh thành công! Bạn nhận được 1.000.000 ${coin}. ${vetify}`));
    }

    if (interaction.commandName === 'so-du') {
        const balance = await db.get(`balance_${userId}`) || 0;
        return interaction.reply(makeEmbed(`💰 Số dư của bạn: **${balance.toLocaleString()} ${coin}**.`));
    }
    ...
    if (interaction.commandName === 'quan-ly-ca') {
        ...
        if (subcommand === 'them') {
            ...
            return interaction.reply(makeEmbed(`✅ Đã thêm **${amount.toLocaleString()} ${coin}** cho <@${member.id}>. Số dư mới: **${newBalance.toLocaleString()} ${coin}**.`));
        }
        ...
        if (subcommand === 'sua') {
            ...
            return interaction.reply(makeEmbed(`✅ Đã sửa số cá của <@${member.id}> thành **${amount.toLocaleString()} ${coin}**.`));
        }
        ...
        if (subcommand === 'xoa') {
            ...
            await db.set(key, newBalance);
            return interaction.reply(makeEmbed(`✅ Đã xóa **${removedAmount.toLocaleString()} ${coin}** của <@${member.id}>. Số dư còn lại: **${newBalance.toLocaleString()} ${coin}**.`));
        }

        if (subcommand === 'xem') {
            return interaction.reply(makeEmbed(`💰 Số dư của <@${member.id}>: **${currentBalance.toLocaleString()} ${coin}**.`));
        }
    }
});
